	...
	public static void main(String[] args) throws IOException
{
	...
	public void myGmailFriends(Model model) {
		runQuery("select DISTINCT ?name where{ ?sub rdf:type <http://org.semweb/assign6/people#GmailPerson> ."+"?sub foaf:name ?name } ", model);
	}
	...
	runQuery("select DISTINCT ?name where{?sub rdf:type <http://org.semweb/assign6/people#EmailPerson> ."+"?sub foaf:name ?name}", model);  //add the query string
	...
	{
	InputStream inFoaf = FileManager.get().open("Ontologies/FOAFfriends.rdf");
	...
	{
	InputStream inFoafInstance = 
	FileManager.get().open("Ontologies/additionalFriends.owl");
